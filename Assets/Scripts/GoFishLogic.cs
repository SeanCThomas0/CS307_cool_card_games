...
public class GoFishLogic : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        gameState = gameStates.REQUEST_CARD_FROM_HAND;
    }
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit.collider)
            {
                if (gameState == gameStates.PICK_CARD_FROM_POOL)
                {
                    PickFromPool(hit.collider.gameObject);
                }
                else if (gameState == gameStates.REQUEST_CARD_FROM_HAND && cardDealer.ContainsCard(playerSingle.GetHand(), hit.collider.gameObject.GetComponent<Card>().numValue, hit.collider.gameObject.GetComponent<Card>().suitValue))
                {
    ...
    {
        List<GameObject> removed = from.RemoveAllNumFromHand(card.GetComponent<Card>().numValue);
        for (int i = 0; i < removed.Count; i++)
        {
            to.AddToHand(removed[i]);
        }
    ...
    private void TakeTurn(GameObject card)
    {
        ...
        bool cardsRemoved = RequestCards(playerBot, playerSingle, card);
